...
class Tags extends Base
{
    ...
    {	
    	if(request()->isPost()){
    		...
    		if(db('Tags')->insert($data)){
    			return $this->success('添加Tag标签成功！','lst','',0);
    		}else{
    			return $this->error('添加Tag标签失败！','','',0);
    		}
    		return;
    	}
        return $this->fetch();
    ...
    public function edit(){
    	...
    	if(request()->isPost()){
    		...
    		if(db('Tags')->update($data)){
    			$this->success('修改Tag标签成功！','lst','',0);
    		}else{
    			$this->error('修改Tag标签失败！','','',0);
    		}
    		return;
    	}
    ...
    public function del(){
		...
		if(db('Tags')->delete(input('id'))){
			$this->success('删除Tag标签成功！','lst','',0);
		}else{
			$this->error('删除Tag标签失败！','','',0);
		}
